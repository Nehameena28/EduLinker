          ...
          <div className="p-4 sm:p-6">
            ...
            {isLoading ? (
              <div className="space-y-3 sm:space-y-4">
                {[...Array(4)].map((_, index) => (
                  <div key={index} className="flex justify-between items-center p-3 sm:p-4 border border-gray-100 rounded-lg sm:rounded-xl">
              ...
              <div className="text-center py-8 sm:py-12">
                ...
                <p className="text-gray-400 text-xs sm:text-sm mt-1">Start uploading notes to see your activity here</p>
              </div>
            ) : (
              <div className="space-y-3 sm:space-y-4">
                {recentActivity.slice(0, showAllActivity ? recentActivity.length : 4).map((activity) => (
                  <div
                    key={activity.id}
                    className="flex justify-between items-center p-3 sm:p-4 hover:bg-gray-50 transition-all duration-200 border border-gray-100 rounded-lg sm:rounded-xl hover:border-custom-blue"
                  >
                    <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-custom-blue/10 to-custom-i-berry/10 rounded-full flex items-center justify-center flex-shrink-0">
                        {activity.type === 'sale' ? (
                          ...
                          <svg className="w-4 h-4 sm:w-5 sm:h-5 text-custom-blue" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                          </svg>
                        )}
                      </div>
                    ...
                    <div className="text-right flex flex-col items-end gap-1 flex-shrink-0">
                      <span className="font-bold text-custom-blue text-sm sm:text-lg">
                        {activity.amount}
                      </span>
